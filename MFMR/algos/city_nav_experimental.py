...
class City:
    def __init__(self, id_no: int, position: np.ndarray, n_p, random: np.random.RandomState, num_places=150, city_size=1) -> None:
        ...
        self.random = random
        self.city_size = city_size
        self.neighbors = []  # type: List[int]
        self.places_positions = [np.zeros(2)] * self.num_places
        self.places_neighbors = []  # type: List[List[int]]
        self.costs = np.ones(shape=(self.num_places, self.num_places)) * np.inf
        self._generate_places()

    def places_connected(self, place1_id, place2_id):
        return self.costs[place1_id, place2_id] < np.inf

    def connect_places(self, place1_id, place2_id):
        # don't allow recalculation of cost
        if not self.places_connected(place1_id, place2_id):
            distance = get_distance(
                self.places_positions[place1_id], self.places_positions[place2_id])
            cost = distance * (1 + self.random.random() * 0.1)
            self.costs[place1_id, place2_id] = cost
            self.costs[place2_id, place1_id] = cost
            self.places_neighbors[place1_id].append(place2_id)
            self.places_neighbors[place2_id].append(place1_id)
    ...
    def _generate_places(self):
        ...
        for place_id in range(self.num_places):
            if place_id == 0:
                self.places_positions[place_id] = self.position
            else:
                self.places_positions[place_id] = self.position + \
                    self.random.random(size=(2,)) * self.city_size
            self.places_neighbors.append([])
            self.costs[place_id, place_id] = 0
            if place_id > 0:
                self.connect_places(place_id, place_id - 1)
            if place_id == self.num_places - 1:
                self.connect_places(place_id, 0)

        for place_id in range(self.num_places):
            nearest_np_places_ids = sorted(
                range(self.num_places), key=lambda p_id: get_distance(self.places_positions[place_id], self.places_positions[p_id]))[1: self.n_p + 1]
            for nearby_place_id in nearest_np_places_ids:
                self.connect_places(place_id, nearby_place_id)
...
class CityNavigation(SearchProblem):
    ...
    def cities_connected(self, city1: City, city2: City):
        return self.costs[city1.id_no, city2.id_no] < np.inf

    def connect_cities(self, city1: City, city2: City):
        # don't allow recalculation of cost
        if not self.cities_connected(city1, city2):
            distance = get_distance(city1.position, city2.position)
            cost = distance + 2
            self.costs[city1.id_no, city2.id_no] = cost
            self.costs[city2.id_no, city1.id_no] = cost
            city1.neighbors.append(city2.id_no)
            city2.neighbors.append(city1.id_no)

    def get_loctions_distance(self, location1, location2):
        city1_id, place1_id = location1
        city2_id, place2_id = location2
        return get_distance(self.cities[city1_id].places_positions[place1_id],
                            self.cities[city2_id].places_positions[place2_id])
    ...
    def reset(self):
        ...
        for city_id in range(self.num_cities):
            city_position = self.random.random(size=(2,)) * self.state_size
            city = City(city_id, city_position, self.n_p,
                        self.random, self.num_places, self.city_size)
            self.costs[city_id, city_id] = 0
            if city_id > 0:
                self.connect_cities(city, self.cities[-1])
            if city_id == self.num_cities - 1:
                self.connect_cities(city, self.cities[0])
            self.cities.append(city)

        for city in self.cities:
            nearest_nc_cities = sorted(
                self.cities, key=lambda c: get_distance(city.position, c.position))[1: self.n_c + 1]
            for nearby_city in nearest_nc_cities:
                self.connect_cities(city, nearby_city)

        self.start_location, self.end_location = (0, 0), (0, 0)
        while self.start_location == self.end_location:
            self.start_location = (self.random.randint(
                self.num_cities), self.random.randint(self.num_places))
            self.end_location = (self.random.randint(
        ...
        self.start_state = self.start_location
        start_end_distance = self.get_loctions_distance(
            self.start_location, self.end_location)
    ...
    def successors(self, state: Tuple[int, int]):
        ...
        cur_city = self.cities[cur_city_id]

        for neighbor_place_id in cur_city.places_neighbors[cur_place_id]:
            action = f'goto ({cur_city_id}, {neighbor_place_id})'
        ...
        if cur_place_id == 0:
            '''can go to other cities as well'''
            for neighbor_city_id in cur_city.neighbors:
                action = f'goto city {neighbor_city_id}'
                next_state = (neighbor_city_id, 0)
                yield {'state': next_state, 'action': action}

    def cost(self, state, action, next_state):
        city1_id, place1_id = state
        city2_id, place2_id = next_state
        if city1_id == city2_id:
            return self.cities[city1_id].costs[place1_id, place2_id]
        else:
            return self.costs[city1_id, city2_id]

    def heuristic(self, state):
        return self.get_loctions_distance(state, self.end_location)
...
if __name__ == "__main__":
    city_nav = CityNavigation(3, 3, 5, 5)
    city_nav.seed(0)
